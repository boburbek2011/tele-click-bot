    ...
    try:
        ...
        if user:
            return web.json_response({
                ...
                'title': user[8],
                'color': user[9],
                'total_clicks': int(user[10]),
                'energy': int(user[11]),
                'max_energy': int(user[12]),
                'click_power': int(user[13]),
                'offline_mining': bool(user[14]),
                'mining_rate': int(user[15]),
                'auto_clicker_level': int(user[17]),
                'active_skin_id': int(user[18]) if user[18] else 0,
                'multiplier': int(user[19])
            })
...
async def handle_click(request):
    try:
        ...
        return web.json_response({
            'success': True,
            'coins': int(updated_user[4]),
            'exp': int(updated_user[5]),
            'level': int(updated_user[6]),
            'energy': int(updated_user[11]),
            'max_energy': int(updated_user[12]),
...
async def buy_diamond(request):
    try:
        ...
        user = await get_user(int(user_id))
        if not user or int(user[4]) < 1500000:
...
async def send_coins(request):
    try:
        ...
        user = await get_user(int(user_id))
        if not user or int(user[4]) < amount:
            return web.json_response({'error': 'Not enough coins'}, status=400)
...
async def buy_shop_item(request):
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            async with db.execute(
                "SELECT id, name, description, category, price, level_required, effect_type, effect_value, emoji FROM shop_items WHERE id = ? AND is_available = 1",
                (int(item_id),)
            ) as cursor:
        ...
        item_id_val = int(item[0])
        item_name = item[1]
        item_desc = item[2]
        item_category = item[3]
        item_price = int(item[4])
        item_level_req = int(item[5])
        effect_type = item[6]
        effect_value = int(item[7])
        item_emoji = item[8]
        
        # Check level
        user_level = int(user[6])
        if user_level < item_level_req:
            return web.json_response({
                'error': f'Bu narsa uchun {item_level_req}-daraja kerak! Sizda: {user_level}'
            }, status=400)
        
        # Check coins
        user_coins = int(user[4])
        if user_coins < item_price:
            return web.json_response({
                'error': f'Yetarli tanga yo\'q! Kerak: {item_price}, Sizda: {user_coins}'
            }, status=400)
        
        # Apply effect
        if effect_type == 'max_energy':
            await update_user_stats(int(user_id), max_energy_delta=effect_value, coins_delta=-item_price)
        elif effect_type == 'click_power':
            await update_user_stats(int(user_id), click_power_delta=effect_value, coins_delta=-item_price)
        elif effect_type == 'auto_clicker':
            await update_user_stats(int(user_id), auto_clicker_delta=effect_value, coins_delta=-item_price)
            ...
            async with aiosqlite.connect(DB_PATH) as db:
                ...
                await db.commit()
            await update_user_stats(int(user_id), coins_delta=-item_price)
        ...
        return web.json_response({
            ...
            'coins': int(updated_user[4]),
            'max_energy': int(updated_user[12]),
            'click_power': int(updated_user[13]),
            'auto_clicker_level': int(updated_user[17]),
            'message': f'✅ {item_name} sotib olindi!'
        })
    except Exception as e:
...
async def get_skins_api(request):
    try:
        ...
        return web.json_response(skins)
    except Exception as e:
...
async def buy_skin(request):
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            async with db.execute(
                "SELECT id, name, emoji, rarity, level_required, price, multiplier FROM skins WHERE id = ? AND is_available = 1",
                (int(skin_id),)
            ) as cursor:
        ...
        skin_id_val = int(skin[0])
        skin_name = skin[1]
        skin_emoji = skin[2]
        skin_rarity = skin[3]
        skin_level_req = int(skin[4])
        skin_price = int(skin[5])
        skin_multiplier = int(skin[6])
        
        # Check level
        user_level = int(user[6])
        if user_level < skin_level_req:
            return web.json_response({
                'error': f'Bu skin uchun {skin_level_req}-daraja kerak! Sizda: {user_level}'
            }, status=400)
        
        # Check coins
        user_coins = int(user[4])
        if user_coins < skin_price:
            return web.json_response({
                'error': f'Yetarli tanga yo\'q! Kerak: {skin_price}, Sizda: {user_coins}'
            }, status=400)
        ...
        await update_user_stats(int(user_id), coins_delta=-skin_price)
        ...
        return web.json_response({
            'success': True,
            'coins': int(updated_user[4]),
            'message': f'✅ {skin_name} skin sotib olindi!'
        })
    except Exception as e:
...
async def admin_add_coins_api(request):
    try:
        ...
        return web.json_response({
            'success': True,
            'user_id': target_id,
            'new_coins': int(user[4]) if user else 0,
...
async def admin_add_exp_api(request):
    try:
        ...
        return web.json_response({
            'success': True,
            'user_id': target_id,
            'new_exp': int(user[5]) if user else 0,
...
async def admin_add_energy_api(request):
    try:
        ...
        return web.json_response({
            'success': True,
            'user_id': target_id,
            'new_energy': int(user[11]) if user else 0,
...
async def place_bid(request):
    try:
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            if amount <= int(auction[5]):
                return web.json_response({'error': f'Bid must be higher than {auction[5]}'}, status=400)
            
            if auction[6] and int(auction[6]) > 0:
                await update_user_stats(int(auction[6]), coins_delta=int(auction[5]))
    ...
    try:
        ...
        for auction in auctions:
            ...
            result.append({
                ...
                'start_price': int(auction[4]),
                'current_bid': int(auction[5]) if auction[5] else int(auction[4]),
